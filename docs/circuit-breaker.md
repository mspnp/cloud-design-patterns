...
- [Retry Pattern](retry.md). The Retry pattern describes how an application can handle anticipated temporary failures when it tries to connect to a service or network resource by transparently retrying an operation that has previously failed. <<RBC: Made a similar comment in the retry pattern file. If you decide to keep the language there we should probably add it back in here as well.>>
