...
The order of the steps in the compensating transaction doesn't necessarily have to be the mirror opposite <<RBC: If this was changed above, use the same term here.>> of the steps in the original operation. For example, one data store might be more sensitive to inconsistencies than another, and so the steps in the compensating transaction that undo the changes to this store should occur first.
